...
model_id = widgets.Dropdown(
    ...
    value='tiny', # ['tiny.en', 'tiny', 'base.en', 'base', 'small.en', 'small', 'medium.en', 'medium', 'large-v1', 'large-v2', 'large-v3', 'large']
    description='Model:',
    disabled=False,
)
...
end_time = time.time()
print("tiny 正常執行時間（英文）：", end_time - start_time, "秒")
...
print(transcription["text"])
end_time = time.time()
print("tiny 正常執行時間（中文）：", end_time - start_time, "秒")
...
end_time = time.time()
print("tiny 加速執行時間（英文）：", end_time - start_time, "秒")

raw_speech = read_wav("train.mp3")
start_time = time.time()
transcribe(raw_speech)
end_time = time.time()
print("tiny 加速執行時間（中文）：", end_time - start_time, "秒")